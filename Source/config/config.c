...
#if defined PANEL_KEY
    #if defined PANEL_6KEY_A11_WZR || defined PANEL_6KEY_A13_LHW
...
static void app_set_relay_pin(panel_cfg_t *panel_cfg, uint8_t relay_num, const gpio_pin_t *relay_map)
{
    panel_cfg->relay_pin[0] = BIT0(relay_num) ? relay_map[0] : DEF;
    panel_cfg->relay_pin[1] = BIT1(relay_num) ? relay_map[1] : DEF;
    panel_cfg->relay_pin[2] = BIT2(relay_num) ? relay_map[2] : DEF;
    panel_cfg->relay_pin[3] = BIT3(relay_num) ? relay_map[3] : DEF;
}
#endif // PANEL_KEY
